...
const CategoryList = ({match, ...props}) => {
	...
	const handleShowStatus = value => {
		if(value !== 'All') {
			...
			const data = utils.filterArray(subCategories, key, value)
			setList(data)
		} else {
	...
	const {data: subCategories, loadingDone: loadingDoneSubCategories} = useAxios({
		...
		setList(res.sub_categories)
		return res.sub_categories
	})
		...
		},
		{
			...
			render: category_id => {
				if(categories?.length){
					const data = utils.filterArray(categories, 'id', category_id)
	...
	const onSearch = e => {
		...
		const searchArray = e.currentTarget.value? list : subCategories
	...
	const addSubCategory = () => {
		history.push(`${match.url}/add-sub-category`)
	}
	...
	return (
		<Card>
			<Flex alignItems="center" justifyContent="between" mobileFlex={false}>
				...
				<div>
				<Button onClick={addSubCategory} type="primary" icon={<PlusCircleOutlined />} block>Add Sub Category</Button>
				</div>
			</Flex>
