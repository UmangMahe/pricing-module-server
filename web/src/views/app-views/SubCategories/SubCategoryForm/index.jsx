...
import { CATEGORY_LIST, PRODUCT_LIST, SUB_CATEGORY_LIST } from '@constants/ApiConstants';
...
const CategoryForm = props => {
	...
	const {isLoading, callback: getCategories} = useAxiosCallback()
	...
	useEffect(() => {
		if (mode === EDIT) {
			...
			getCategories({
				method: 'GET',
				url: `${SUB_CATEGORY_LIST}/${produtId}`,
				success:(res)=>{
					console.log('res', res)
					const {sub_categories} = res
					form.setFieldsValue({
						name: sub_categories.name,
						description: sub_categories.description,
						icon: sub_categories.icon,
						category_id: sub_categories.category_id
						// variation: sub_categories.variation,
						// shipping: sub_categories.shipping,
					})
					setImage(Utils.getImgUrl(sub_categories.icon))
				}
			})
		}
	}, [mode, form, param]);
		...
		form.validateFields().then(values => {
			...
			setTimeout(() => {
				...
				if (mode === EDIT) {
					...
					upDateProduct({
						method: 'POST',
						url: `${SUB_CATEGORY_LIST}/${param.id}`,
						data: form,
						success: (res)=>{
				...
				<PageHeaderAlt className="border-bottom" overlap>
					<div className="container">
						<Flex className="py-2" mobileFlex={false} justifyContent="between" alignItems="end">
							<h2 className="mb-3">{mode === 'ADD' ? 'Add New Sub Category' : `Edit Sub Category`} </h2>
							<div className="mb-3">
